...
def add_guest(list_of_guests):
    list_of_guests.append(simpledialog.askstring("Prompt", "What is your name?"))
    return list(list_of_guests)
...
def remove_guest(list_of_guests):
    list_of_guests.remove(simpledialog.askstring("Prompt", "What is your name?"))
    return list(list_of_guests)
...
def print_guests(list_of_guests):
    if len(list_of_guests) == 0:
        messagebox.showinfo("Message", "There are no guests")
    else:
        idx = 1
        msg = ""
        for guest in list_of_guests:
            msg += "Guest " + str(idx) + ". " + guest + "\n"
            idx += 1
